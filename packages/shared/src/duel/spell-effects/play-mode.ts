 ...
 *
 * Derived from the effect table, never from `tipo` alone — the dataset cannot
 * tell a terrain from Raigeki, since both are `tipo: "magica"`,
 * `classe: "Magic"`. Lives in `shared` because `apps/web`'s intent state
 * machine needs it to route a hand selection, and it may not import
 * `packages/engine`.
 */
