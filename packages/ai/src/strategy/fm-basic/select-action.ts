...
export function selectFmBasicAction(input: Readonly<{ state: PublicDuelState; legalResult: LegalCandidateFilterResult; parameters: FmBasicParameters }>): DuelAction {
  if (input.legalResult.kind === "fallback") return input.legalResult.action;
  const candidates = input.legalResult.candidates;
  return selectSummon(input.state, candidates, input.parameters)?.action
