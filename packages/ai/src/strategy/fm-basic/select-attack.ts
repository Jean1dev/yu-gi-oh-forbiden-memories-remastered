...
export function selectAttack(state: PublicDuelState, candidates: readonly LegalCandidate[], parameters: FmBasicParameters): LegalCandidate | undefined {
  const attacks = candidates.filter((candidate) => candidate.action.type === "declare_attack");
  const direct = attacks.find((candidate) => candidate.action.type === "declare_attack" && candidate.action.targetZoneIndex === undefined);
  if (direct !== undefined) return direct;
  return attacks.find((candidate) => {
    if (candidate.action.type !== "declare_attack" || candidate.action.targetZoneIndex === undefined) return false;
