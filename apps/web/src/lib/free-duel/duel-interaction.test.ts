...
describe("duel interaction reducer", () => {
  ...
  it("declares targeted and direct attacks", () => {
    ...
    const choosingAttacker = reduceIntent(
      { kind: "idle" },
      { type: "invoke_slot", id: "attack" },
      battle,
    );
    ...
    expect(
      reduceIntent(directIntent, { type: "invoke_slot", id: "direct_attack" }, directBattle).action,
    ).toEqual({
      type: "declare_attack",
      attackerZoneIndex: 0,
    });
  ...
  it("changes position from a player monster in battle", () => {
    ...
    const choosing = reduceIntent(
      { kind: "idle" },
      { type: "invoke_slot", id: "change_position" },
      battle,
    );
    expect(
      reduceIntent(
        choosing.intent,
        { type: "activate_zone", reference: zoneReference("P1", "monster", 4) },
        battle,
      ).action,
  ...
  it("ignores inapplicable events and resets after producing an action", () => {
    ...
    expect(
      reduceIntent(base, { type: "choose_position", position: "attack_face_up" }, state()).intent,
    ).toBe(base);
    expect(
      reduceIntent({ kind: "card_selected", handIndex: 0 }, { type: "cancel" }, state()).intent,
    ).toEqual({
      kind: "idle",
    });
    expect(
...
describe("duel affordances", () => {
  ...
  it("mirrors hand play, summon and spell placement guards", () => {
    ...
    expect(
      describeAffordances({
        ...
        isPlayerTurn: true,
        busy: false,
        intent: { kind: "card_selected", handIndex: 1 },
      }).canSummon,
    ).toBe(false);
    expect(
      describeAffordances({
  ...
  it("mirrors attack, direct attack and position change guards", () => {
    const firstTurn = withMonster(
      state({ phase: "battle", turn: 1 }),
      zoneReference("P1", "monster", 0),
    );
    expect(
      describeAffordances({
        state: firstTurn,
    ...
    const faceDownAttack = withMonster(
      state({ phase: "battle" }),
      zoneReference("P1", "monster", 0),
      monster,
      {
  ...
  it("returns exactly three slots and keeps phase advance in the primary slot", () => {
    ...
    const slots = describeActionSlots(
      { intent: { kind: "card_selected", handIndex: 0 }, state: state() },
      affordances,
    );
  ...
  it("classifies zones for the current intent", () => {
    ...
    expect(
      zoneAffordance(
        base,
        { kind: "choosing_target", attackerZoneIndex: 0 },
        zoneReference("P2", "monster", 1),
      ),
    ).toBe("target");
    expect(
