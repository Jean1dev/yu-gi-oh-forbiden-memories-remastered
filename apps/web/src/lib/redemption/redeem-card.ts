import { CardRedemptionIntentSchema, DomainError, err, ok, type CardRedemptionOutcome, type PendingCardRedemption, type Result } from "@yugioh/shared";
...
export type RedemptionQueue = Readonly<{ enqueue(item: PendingCardRedemption): Promise<void>; listPending(playerId: string): Promise<readonly PendingCardRedemption[]>; remove(redemptionId: string): Promise<void> }>;
