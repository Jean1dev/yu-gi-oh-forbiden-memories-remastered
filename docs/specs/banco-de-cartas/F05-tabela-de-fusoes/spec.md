...
| 10 | **PENDÊNCIA DE DADOS:** os valores reais das receitas de fusão do Forbidden Memories **não existem no repositório e não são inventados aqui**. Enquanto `fusoes.json` permanecer `[]`, toda consulta (`buscarPorParDeMateriais`, `buscarPorParDeClasses`, `listarPorResultado`) devolve "sem fusão conhecida" (`undefined` ou lista vazia), e o critério de aceite de fidelidade do PRD §9 F05 ("as fusões correspondem às do jogo original") permanece **bloqueado** até o mantenedor fornecer os valores. | PRD §6 F05 Capabilities + §9 (critério marcado "Pendente"); Fase 0.4 do skill; `arquitetura.md` §4.3; ADR-003 `[PRECISA DE ENTRADA]` | pendente — aguarda dado externo |
